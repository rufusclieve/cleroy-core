...
export default function ProjectDiscoveryWorkspace({ isOpen, onClose }: ProjectDiscoveryWorkspaceProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      setCurrentStep(2);
      setErrors({ phoneOrWhatsapp: `Please provide your ${formData.preferredContactMethod.toLowerCase()} number` });
      return;
    }
    ...
    const messageText = formData.ideaText.trim() || 
      (formData.selectedChips.length > 0 
        ? `Interested in capabilities: ${formData.selectedChips.join(", ")}` 
        : `New ${formData.projectType || "Website"} enquiry from ${formData.fullName}`);
    ...
    const payload = {
      ...
      whatsappNumber: formData.phoneOrWhatsapp.trim(),
      phoneNumber: formData.phoneOrWhatsapp.trim(),
      ...
      productDescription: messageText,
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        setSubmitError(
          result.error ||
          "Unable to send message. Please verify your details or email cleroyhq@gmail.com directly."
        );
      }
    } catch (err: any) {
      ...
      if (err.name === "AbortError") {
        setSubmitError("The request timed out. Please check your network connection and try again.");
      } else {
        setSubmitError("Unable to connect to the email server. Please try again or email cleroyhq@gmail.com directly.");
      }
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[99999] w-full h-full h-[100dvh] max-h-[100dvh] bg-[#050505] text-[#F5EFE7] font-sans overflow-hidden flex flex-col select-none">
      ...
      {/* MAIN CONTAINER */}
      <main className={`relative z-20 flex-grow overflow-y-auto custom-scrollbar flex flex-col justify-start sm:justify-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 md:py-6 ${currentStep === 1 ? 'max-w-5xl' : 'max-w-3xl'} mx-auto w-full transition-all duration-300 pb-20 sm:pb-4`}>
        <AnimatePresence mode="wait">
          ...
          {currentStep === 2 && (
            <motion.div
              ...
              className="space-y-4 sm:space-y-5 my-auto sm:my-auto py-2 sm:py-4 w-full max-w-2xl mx-auto"
            >
              {/* HEADING */}
          ...
          {currentStep === 3 && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -15 }}
              transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
              className="space-y-3.5 sm:space-y-4 my-auto sm:my-auto py-2 sm:py-4 w-full max-w-2xl mx-auto"
            >
              {/* HEADING */}
              <div className="space-y-1">
                <span className="font-mono text-[10px] sm:text-xs text-[#E85002] tracking-[0.2em] uppercase font-bold block">
            ...
            >
              ...
              {submitError && (
                <motion.div
                  ...
                  className="p-3.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-200 text-xs font-mono flex items-center gap-2.5"
                >
      ...
      {currentStep > 1 && currentStep < 5 && (
        <footer className="relative z-30 shrink-0 px-4 sm:px-8 md:px-12 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] border-t border-white/10 bg-[#050505]/95 backdrop-blur-xl flex items-center justify-between">
          {currentStep > 1 ? (
            <button
              type="button"
              onClick={handleBack}
